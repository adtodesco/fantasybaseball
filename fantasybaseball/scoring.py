        ...
        if stat in custom_stats:
            for s, c in custom_stats[stat].items():
                if s in coefficients:
                    ...
                    logger.debug(
                        "Cannot find stat '{}' (used to calculate custom stat '{}') in stat cols.".format(s, stat)
                    )
        elif stat in stat_cols:
            coefficients[stat] += float(points)
        else:
            logger.debug("Cannot find stat '{}' in stat cols or custom stats.".format(stat))
...
def _total_bases(**kwargs):
    assert all(stat in kwargs["stats"] for stat in [Stat.H.value, Stat.Dbl.value, Stat.Trp.value, Stat.HR.value])
    total_bases = (
...
def _double_plays(**kwargs):
    ...
    return round(average_gdp_per_ab * int(kwargs["stats"][Stat.AB.value])) * int(kwargs["points"])
...
def _hit_batters(**kwargs):
    ...
    return round(average_hb_per_ip * int(kwargs["stats"][Stat.AB.value])) * int(kwargs["points"])
...
def _quality_starts(**kwargs):
    ...
    return round(average_qs_per_gs * int(kwargs["stats"][Stat.AB.value])) * int(kwargs["points"])
...
def _complete_games(**kwargs):
    ...
    return round(average_cg_per_gs * int(kwargs["stats"][Stat.GS.value])) * int(kwargs["points"])
...
def _shut_outs(**kwargs):
    ...
    return round(average_so_per_gs * int(kwargs["stats"][Stat.AB.value])) * int(kwargs["points"])
...
def _blown_saves(**kwargs):
    ...
    return round(average_bs_per_sv * int(kwargs["stats"][Stat.SV.value])) * int(kwargs["points"])
